...
function attachSocketServer(server) {
  ...
  logInfo({
    ...
    intent:
      "Expose live authenticated realtime channels for chat updates",
  });
    ...
    try {
      ...
      if (!token) {
        logError({
          ...
          intent:
            "Reject socket connections that do not send a bearer access token",
          classification: "AUTHENTICATION_ERROR",
          error_code: "AUTH_ACCESS_TOKEN_MISSING",
          resolution_hint:
            "Send a valid bearer access token during the socket handshake",
      ...
      logError({
        ...
        operation: "SocketIoAuth",
        intent:
          "Reject socket connections that present an invalid or expired bearer access token",
        classification:
  ...
  io.on("connection", (socket) => {
    ...
    logInfo({
      ...
      intent:
        "Accept an authenticated socket client and join the role and user rooms",
      userId: authUser.id,
    ...
    socket.on("disconnect", (reason) => {
      logInfo({
        ...
        intent:
          "Confirm the authenticated realtime client disconnected cleanly",
        userId: authUser.id,
        userRole: authUser.role || "-",
        reason,
      });
...
function emitInternalChatDirectoryUpdated(
  userId = null,
) {
  emitToRooms(
    new Set([
