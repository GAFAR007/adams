...
function getAiProviderStatus() {
  if (
    ...
    return {
      provider: env.aiProvider || "configured",
...
function logAiProviderStatus() {
  ...
  logInfo({
    ...
    layer: "service",
    operation: "AiProviderStatus",
    intent:
      "Report the active AI provider and model for public concierge replies at backend startup",
...
function resolveServiceName(
  ...
  const service = (
    Array.isArray(profile?.serviceLabels)
...
function resolveNextStep(payload) {
  if (
    !normalizeText(payload.serviceKey) &&
...
function buildStepPrompt(step, languageCode) {
  const labels = {
    en: {
      ...
      email:
        "What email address should we use for your booking access?",
      phone:
        "What phone number should the team use if they need to reach you quickly?",
      password:
        "Please create a password with at least 8 characters for your customer access.",
      done: "Everything is ready. Confirm below and I will create your secure customer access so you can continue the booking.",
    },
    de: {
      service:
      ...
      done: "Alles ist bereit. Bestätigen Sie unten, dann erstelle ich Ihren sicheren Kundenzugang und Sie können mit der Buchung weitermachen.",
    },
  };

  return (
    labels[languageCode][step] ||
    labels[languageCode].firstName
  );
}
...
async function requestAiReply({
  ...
  const prompt = [
    ...
    firstName
      ? `Use the first name ${firstName} naturally if it helps.`
  ...
  logInfo({
    ...
    intent:
      "Generate the public service-concierge reply through the configured AI provider",
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logError({
        ...
        operation: "PublicServiceConciergeReply",
        intent:
          "Capture the AI provider failure before the flow falls back to a deterministic reply",
        classification:
          ERROR_CLASSIFICATIONS.PROVIDER_OUTAGE,
        error_code:
          "PUBLIC_SERVICE_CONCIERGE_PROVIDER_REJECTED",
        resolution_hint:
    ...
    logInfo({
      ...
      operation: "PublicServiceConciergeReply",
      intent:
        "Confirm the AI provider returned a usable concierge reply",
      provider: env.aiProvider,
      model: env.aiModelDefault,
    });
    ...
    logError({
      ...
      operation: "PublicServiceConciergeReply",
      intent:
        "Capture unexpected AI provider errors before the flow falls back safely",
      classification:
        ERROR_CLASSIFICATIONS.PROVIDER_OUTAGE,
      error_code:
        "PUBLIC_SERVICE_CONCIERGE_PROVIDER_FAILED",
      resolution_hint:
...
async function generatePublicServiceConciergeReply(
  payload,
  logContext,
) {
  ...
  logInfo({
    ...
    operation: "PublicServiceConciergeReply",
    intent:
      "Generate a public booking-assistant reply for the service-intake chat",
  });
  ...
  logInfo({
    ...
    operation: "PublicServiceConciergeReply",
    intent:
      "Load the public company profile so the concierge reply uses live business context",
  });
  ...
  if (!profile) {
    throw new AppError({
      ...
      errorCode:
        "PUBLIC_COMPANY_PROFILE_NOT_FOUND",
      resolutionHint:
        "Seed the company profile before using the public booking assistant",
  ...
  logInfo({
    ...
    operation: "PublicServiceConciergeReply",
    intent:
      "Confirm the public company profile is available for concierge context",
  });

  const justCapturedStep =
    normalizeText(payload.justCapturedStep) ||
    "service";
  ...
  return {
    message:
      "Public service concierge reply generated successfully",
    assistant: {
      name: ASSISTANT_NAME,
      reply,
      nextStep,
