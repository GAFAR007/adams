...
const publicCompanyProfileController = asyncHandler(async (req, res) => {
  const logContext = buildRequestLog(req, {
    layer: 'controller',
    operation: 'PublicGetCompanyProfile',
    intent: 'Return the public company profile for the landing page',
  });

  const result = await publicService.getCompanyProfile(logContext);
...
const publicServiceConciergeReplyController = asyncHandler(async (req, res) => {
  const logContext = buildRequestLog(req, {
    layer: 'controller',
    operation: 'PublicServiceConciergeReply',
    intent: 'Return the next customer-care reply for the public booking chat',
  });

  const result = await serviceConciergeService.generatePublicServiceConciergeReply(
    req.body,
    logContext,
  );
  res.status(200).json(result);
